// Full end-to-end demo seed for Momni — hosts, guests, bookings across every status,
// visits (check-in/out), message threads, two-way reviews, the demo Circle, and Campfire.
// Idempotent: keyed on demo emails, so re-running won't duplicate. Safe to run on the live DB.
//
//   node seed-demo.js
//
// To REMOVE all demo data later (before real launch), run:  node seed-demo.js --wipe
const { db, seedDemoCircle } = require('./db');
...
function wipe() {
  ...
  if (!ids.length) { console.log('No demo users to wipe.'); return; }
  ...
  console.log(`Wiped ${ids.length} demo users and all their links/reviews/messages/visits/campfire.`);
}

if (process.argv.includes('--wipe')) { wipe(); process.exit(0); }

if (db.prepare("SELECT 1 FROM users WHERE email = 'emily@example.com'").get()) {
  console.log('Demo already seeded (emily@example.com exists). Re-ensuring the Circle, then exiting.');
  seedDemoCircle(db);
  process.exit(0);
}
...
// ---------- bookings (links) across the full lifecycle ----------
// status: requested | confirmed | completed | declined | cancelled ; care_type: one-time | recurring | overnight
...
function link(guest, host, care_type, details, status, daysAgo) {
  const info = mkLink.run(id(guest), id(host), care_type, JSON.stringify(details), status, ACK, `-${daysAgo} days`, `-${daysAgo} days`);
  return info.lastInsertRowid;
}
const L = {};
// 5 completed cares for Sarah (earns Super Host: 5+ completed & 4.5★+)
...
// ---------- visits (check-in / check-out timeline) ----------
const pad = (n) => String(n).padStart(2, '0');
function ymd(daysAgo) { const d = new Date(Date.now() - daysAgo * 86400000); return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`; }
function dt(daysAgo, h, m) { const d = new Date(Date.now() - daysAgo * 86400000); d.setHours(h, m, 0, 0); return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(h)}:${pad(m)}:00`; }
...
// completed visits on completed links (checked in and out)
...
// active confirmed link (Maren): one completed overnight + one upcoming scheduled
...
// ---------- messages (the thread between mamas) ----------
...
// ---------- reviews (member content, both directions on completed cares) ----------
const mkReview = db.prepare(`INSERT INTO reviews (link_id,author_id,subject_id,rating,body,created_at) VALUES (?,?,?,?,?, datetime('now', ?))`);
function review(linkId, author, subject, rating, body, daysAgo) {
  mkReview.run(linkId, id(author), id(subject), rating, body, `-${daysAgo} days`);
}
// Sarah's completed cares — guest→host and host→guest
...
// ---------- demo Circle (Sarah leads Orem Moms Circle) + Campfire ----------
seedDemoCircle(db);
...
if (db.prepare('SELECT COUNT(*) c FROM campfire_posts').get().c === 0) {
  ...
  const cmt = db.prepare(`INSERT INTO campfire_comments (post_id,user_id,body,created_at) VALUES (?,?,?, datetime('now','-2 days'))`);
  cmt.run(p1, id('dana@example.com'), 'Yes please — I rebook the same two mamas constantly.');
}
...
console.log('   links   :', n('SELECT COUNT(*) c FROM links'), '(across requested/confirmed/completed/declined/cancelled)');
...
const sarah = db.prepare("SELECT id FROM users WHERE email='sarah@example.com'").get().id;
const done = n(`SELECT COUNT(*) c FROM links WHERE host_id=${sarah} AND status='completed'`);
const rv = db.prepare(`SELECT AVG(rating) a, COUNT(*) n FROM reviews WHERE subject_id=${sarah}`).get();
console.log(`   Sarah: ${done} completed cares, ${rv.n} reviews, avg ${rv.a ? rv.a.toFixed(2) : '—'}★ → Super Host badge: ${done>=5 && rv.a>=4.5 ? 'YES' : 'no'}`);
